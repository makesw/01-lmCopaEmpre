...
<div class="page-container">
	...
	<div class="main-container">
		...
		<div class="main-content">
			<form id="formJuegos">
			<h1 class="page-title">Administración / Pagos Inscripción</h1>
			<div class="row">			
			<div class="col-lg-12">
			...
			<h1 class="page-title">Listado de pagos de inscripción por equipo :</h1>		
			<div class="row">
			<div class="col-lg-12">
				<div class="class="panel panel-minimal"t">
					<div class="panel-body">
						<div id="divPagosInscrip"></div>						
					</div>
				</div>
			</div>
			</div>		
...
$(document).ready(function(){
   $("#cmbComp").change(function () {
	 idCompSel = $(this).val();
	 $("#divPagosInscrip").load('ajaxPagosInscripcion.php?opt=1&idComp='+idCompSel); 
   })
});	
function fnAddPago( idEquipo ) {
	$("#hdIdComp").val($("#cmbComp").val());
	$("#hdIdEqui").val(idEquipo);
	$("#title-regAbono").text("Registrar Pago Inscripción");
...
function fnAddDto( idEquipo ) {
	...
	$("#hdIdEqui").val(idEquipo);
	$("#hdIdDto").val(1);	
	$("#title-regAbono").text("Registrar Descuento Inscripción");	
	$('#modal-pag').modal('show');
}
jQuery( document ).on( 'submit', '#formRegistrarAbono', function ( event ) {
	$.ajax( {
		...
		success: function ( data ) {
			//console.log( data );
			location.href = './pagosInscripcion.php?idComp='+$("#cmbComp").val();
		},
		error: function ( data ) {
			//console.log( data );
		},
...
if( idCompRld != 0 ){
	...
	$("#divPagosInscrip").load('ajaxPagosInscripcion.php?opt=1&idComp='+idCompRld); 
}
function delDctos( idEquipo, idComp ) {
	if ( confirm( 'Confirma Eliminar?' ) ) {
		$.ajax( {
			...
			success: function ( data ) {
				//console.log( data );
				location.href = './pagosInscripcion.php?idComp='+idComp;
				//location.reload();
			},
			error: function ( data ) {
				//console.log( data );
